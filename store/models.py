...
class UserProfile(models.Model):
    ...
    country = models.CharField(max_length=100, blank=True, null=True, help_text="Country of residence")
...
class SellerProfile(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    region = models.CharField(max_length=100, blank=True, help_text="State/Region/Province for delivery & tax calculations")
    payment_number = models.CharField(max_length=20, blank=True, help_text="Mobile money or bank-linked number for receiving payments")
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending', help_text="Current verification status")
    verified_at = models.DateTimeField(null=True, blank=True, help_text="Timestamp when admin approved")
    rejected_reason = models.TextField(blank=True, default='', help_text="Reason for rejection (visible to seller)")
    admin_notes = models.TextField(blank=True, default='', help_text="Internal admin notes (not visible to seller)")
...
class Product(models.Model):
    ...
    warranty = models.CharField(max_length=100, blank=True, help_text="e.g., 1 Year Manufacturer Warranty")
    
    # VR / 3D fields
    vr_type = models.CharField(max_length=20, choices=[('3d_model', '3D Model'), ('360_image', '360 Image')], blank=True, null=True)
    vr_supabase_path = models.CharField(max_length=500, blank=True, null=True, help_text="Supabase storage path for the VR file")
    
    # Basic Product Info
    price = models.DecimalField(max_digits=10, decimal_places=2)
    stock = models.PositiveIntegerField(default=0, help_text="Global stock count (used if size is not required)")
    
    # ✅ NEW: Size & Dimension Fields
    requires_size = models.BooleanField(default=False, help_text="Check if product has size options (e.g., shoes, clothing)")
    size_stock = models.JSONField(default=dict, blank=True, help_text="JSON mapping of size to quantity. Example: {'S': 10, 'M': 5}")
    ...
    def available_sizes(self):
        """Returns list of sizes with stock > 0"""
        if not self.requires_size: return []
        return [s for s, q in (self.size_stock or {}).items() if q > 0]
...
class OrderItem(models.Model):
    ...
    size = models.CharField(max_length=50, blank=True, help_text="Selected size for this item") # ✅ ADDED
...
def sync_user_profile(sender, instance, created, **kwargs):
    ...
    _sync_to_supabase_prof(instance.user.id, {
        "username": instance.user.username, "email": instance.user.email,
        "whatsapp_number": instance.whatsapp_number, "bio": instance.bio,
        "avatar_url": avatar_url, "updated_at": timezone.now().isoformat()
    })
...
class Region(models.Model):
    ...
    code = models.CharField(max_length=10, blank=True, help_text="Short code")
    is_active = models.BooleanField(default=True)
